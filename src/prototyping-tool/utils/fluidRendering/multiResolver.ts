...
*/
function getUrlResolver(options: any): IUrlResolver {
  switch (options.mode) {
    case "tinylicious":
...
export class MultiUrlResolver implements IUrlResolver {
  ...
  public async createRequestForCreateNew(fileName: string): Promise<IRequest> {
    switch (this.options.mode) {
      case "tinylicious":
        return (this.urlResolver as InsecureUrlResolver).createCreateNewRequest(
          fileName
        );
      default:
        // Local
        return (this.urlResolver as LocalResolver).createCreateNewRequest(
          fileName
        );
    }
  }
