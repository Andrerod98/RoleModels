...
export const FluidLoader = (
  props: React.PropsWithChildren<FluidLoaderProps>
) => {
...
async function loadFluidContainer(
  ...
  const { loader, urlResolver } = await getLoader(
    factory,
    fromContainer.id,
    options
  );
  const documentLoadUrl = await urlResolver.getAbsoluteUrl(
    fromContainer.resolvedUrl,
    ""
  );
